...
func (s *Server) fillClusterConfigsSubStep(_ step.OutStreams, request *idl.InitializeRequest) error {
	...
	s.TargetInitializeConfig, err = assignPorts(s.Source, ports)
	if err != nil {
		return err
	}
...
func assignPorts(source *utils.Cluster, ports []int) (InitializeConfig, error) {
	if len(ports) == 0 {
	...
	return userSpecifiedPorts(source, ports)
}

// can return an error if we run out of ports to use
func userSpecifiedPorts(source *utils.Cluster, ports []int) (InitializeConfig, error) {
...
// for the master.
func defaultTargetPorts(source *utils.Cluster) InitializeConfig {
	targetInitializeConfig := InitializeConfig{}
