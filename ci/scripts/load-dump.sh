...
echo 'Dropping unique and primary keys...'
...
# todo: Getting a parse error for columns query. Hardcoding below for now.
#columns=$(ssh -n mdw "
#    set -x
#
#    source /usr/local/greenplum-db-old/greenplum_path.sh
...
ssh -n mdw "
    ...
    psql regression -c 'ALTER TABLE abstime_tbl DROP COLUMN f1 CASCADE;'
    psql regression -c 'ALTER TABLE money_data DROP COLUMN m CASCADE;'
    psql regression -c 'ALTER TABLE reltime_tbl DROP COLUMN f1 CASCADE;'
    psql regression -c 'ALTER TABLE tinterval_tbl DROP COLUMN f1 CASCADE;'
